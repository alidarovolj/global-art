...
const {form, artworkImages} = storeToRefs(artistArtworksStore);
...
const pushPhoto = (index, photo) => {
  form.value.art_item_images[index] = photo.temp_media_token;
  artworkImages.value[index] = photo.base64;
};
...
const deletePhoto = (index) => {
  form.value.art_item_images.splice(index, 1);
...
<template>
  <div v-if="!pending && form && user.result">
    <form>
      <div class="flex flex-col">
        <div class="w-full">
          ...
          <div class="flex flex-col md:flex-row gap-20">
            <div class="w-full md:min-w-[650px] md:max-w-[650px]">
              <div class="mb-10 pb-10 border-b border-[#E3E3E3] w-full">
                ...
                <div>
                  <div
                      :class="{ 'border-red-500 border': vuelidate.art_item_images.$error }"
                      class="h-60 p-3 py-12 rounded-2xl relative bg-[#F7F7F7] flex flex-col items-center justify-center mb-2"
                  >
                    <div
                        v-if="artworkImages[0]"
                        class="absolute left-1/2 -translate-x-1/2 top-0 w-max h-full opacity-40 py-5 z-0"
                    >
                      <img :src="artworkImages[0]" alt="" class="h-full w-auto">
                    </div>
                    <div class="flex flex-col gap-4 items-center relative z-20">
              ...
              <div class="mb-10 pb-10 border-b border-[#E3E3E3] w-full">
                ...
                <div class="flex flex-col mb-6">
                  ...
                  <input id="artwork_title" v-model="form.art_item_title"
                ...
                <div class="flex flex-col mb-6">
                  ...
                  <textarea id="description" v-model="form.art_item_description"
                  ...
                  <div class="flex justify-between items-center text-sm text-secondaryText">
                    <p v-if="form.art_item_description">
                      {{ form.art_item_description.length }} / 40
                    </p>
                    <p class="text-end">
                ...
                <div class="md:flex justify-between gap-6 mb-6">
                  <div class="flex flex-col w-full md:w-[74%] mb-6 md:mb-0">
                    ...
                    <input id="made_in" v-model="form.art_item_params.location"
                  ...
                  <div class="flex flex-col w-full md:w-fourth">
                    ...
                    <input id="yearMade" v-model="form.art_item_params.year"
                ...
                <div class="flex flex-col">
                  ...
                  <select id="signature_type" v-model="form.art_item_params.signature_id"
          ...
          <div class="flex flex-col md:flex-row gap-20">
            <div class="w-full md:min-w-[450px] md:max-w-[450px] md:min-w-[650px] md:max-w-[650px]">
              <div class="mb-10 pb-10 border-b border-[#E3E3E3]">
                ...
                <div class="flex flex-col mb-6">
                  ...
                  <select id="category" v-model="form.art_item_category_id"
                ...
                <div class="flex flex-col mb-6">
                  ...
                  <input id="rights" v-model="form.art_item_params.rights"
                ...
                <div class="flex flex-col mb-6">
                  ...
                  <select id="rarity" v-model="form.art_item_params.rarity_id"
                ...
                <div class="flex flex-col mb-6">
                  ...
                  <select id="auth_cert" v-model="form.art_item_params.auth_cert_id"
                ...
                <div class="flex flex-col mb-6">
                  ...
                  '!bg-black text-white': form.art_item_styles.includes(
                    item.id,
                  ),
                       ...
                       @click="addElementToArray(form.art_item_styles, item.id)">
                ...
                <div class="flex flex-col mb-6">
                  ...
                  <div class="flex flex-wrap gap-4 text-sm">
                    <p v-for="(item, index) in sortedMaterials" :key="index"
                       :class="[{
                  '!bg-black text-white': form.art_item_materials.includes(
                    item.id,
                  ),
                }, { 'border-red-500 border': vuelidate.art_item_materials.$error }]"
                       class="w-max bg-gray-200   cursor-pointer capitalize hover:bg-black hover:text-white transition-all rounded-full px-4 py-2"
                       @click="addElementToArray(form.art_item_materials, item.id)">
                      {{ item.translations[cur_lang].title }}
                    </p>
                  </div>
                </div>
                <div class="flex flex-col mb-6">
                  ...
                  '!bg-black text-white': form.art_item_subjects.includes(
                    item.id,
                  ),
                }, { 'border-red-500 border': vuelidate.art_item_subjects.$error }]"
                       class="w-max bg-gray-200   cursor-pointer capitalize hover:bg-black hover:text-white transition-all rounded-full px-4 py-2"
                       @click="addElementToArray(form.art_item_subjects, item.id)">
                      {{ item.translations[cur_lang].title }}
                    </p>
                  </div>
                </div>
              ...
              <div class="mb-10 pb-10 border-b border-[#E3E3E3]">
                ...
                <div class="flex mb-6">
                  <div class="flex border p-2 border-[#E5E7EB]  rounded-lg overflow-hidden">
                    <input id="framed" v-model="form.art_item_params.frame" :value="true" checked class="radio-option"
                           name="frame" type="radio"/>
                    <label for="framed">{{ $t("addArtwork.framed") }}</label>
                    <input id="unframed" v-model="form.art_item_params.frame" :value="false" class="radio-option"
                           name="frame" type="radio"/>
                ...
                <div>
                  ...
                  <div class="md:flex justify-between mb-6">
                    <div class="flex flex-col w-full md:w-fourth mb-6 md:mb-0">
                      ...
                      <input id="width" v-model="form.art_item_params.size.width"
                             :class="{ 'border-red-500': vuelidate.art_item_params.size.width.$error }"
                             :placeholder="$t('addArtwork.width')"
                             class="py-[14px] px-4 border border-bColor rounded-lg   "
                             type="number"
                             @input="form.art_item_params.size.width = $event.target.value.replace(/\D/g, '')"
                    ...
                    <div class="flex flex-col w-full md:w-fourth mb-6 md:mb-0">
                      ...
                      <input id="height" v-model="form.art_item_params.size.height"
                             :class="{ 'border-red-500': vuelidate.art_item_params.size.height.$error }"
                             :placeholder="$t('addArtwork.height')"
                             class="py-[14px] px-4 border border-bColor rounded-lg   "
                             type="number"
                             @input="form.art_item_params.size.height = $event.target.value.replace(/\D/g, '')"
                             @keydown="preventDotComma($event)"/>
                    </div>
                    <div class="flex flex-col w-full md:w-fourth mb-6 md:mb-0">
                      ...
                      <input id="length" v-model="form.art_item_params.size.length"
                             :class="{ 'border-red-500': vuelidate.art_item_params.size.length.$error }"
                             :placeholder="$t('addArtwork.depth')"
                             class="py-[14px] px-4 border border-bColor rounded-lg   "
                             type="number"
                             @input="form.art_item_params.size.length = $event.target.value.replace(/\D/g, '')"
                             @keydown="preventDotComma($event)"/>
                    </div>
                  ...
                  <div class="flex flex-col w-full md:w-fourth mb-6 md:mb-0">
                    ...
                    <div class="flex gap-2 items-center">
                      <input id="length" v-model="form.art_item_params.size.weight"
                             :class="{ 'border-red-500': vuelidate.art_item_params.size.weight.$error }"
                             :placeholder="$t('addArtwork.weight')"
                             class="py-[14px] px-4 border border-bColor rounded-lg   "
                             type="number"
                             @input="form.art_item_params.size.weight = $event.target.value.replace(/\D/g, '')"
                             @keydown="preventDotComma($event)"/>
                      <p>
                        {{ $t("addArtwork.kg") }}
                      </p>
                  ...
                  <div class="md:flex justify-between mb-6">
                    <div class="flex flex-col w-full">
                      ...
                      <div class="flex items-center gap-2">
                        <input id="price" v-model="form.art_item_base_price"
