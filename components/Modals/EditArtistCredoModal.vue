...
const v$ = useVuelidate({artist_credo: {required, minLength: minLength(40)}}, form)
  ...
  <div class="modal-box">
    ...
    <form class="flex flex-col gap-2">
      <div class="flex flex-col mb-10">
            <textarea
                v-model="form.artist_credo"
                :class="{ 'border-red-500': v$.artist_credo.$error && v$.artist_credo.$dirty}"
        ...
        <div class="flex justify-between items-center text-sm text-secondaryText">
          <p v-if="form.artist_credo"> {{ form.artist_credo.length }} / 40 </p>
          <p> {{ $t("artistRegistration.modals.editAbout.minimumCharacters") }} </p>
        </div>
      </div>
    </form>
    <div class="mt-5">
