...
const declineOrderLocal = async () => {
  ...
  if (commissions.resultStatus.success) {
    toasts.showToast("success", "Custom order declined successfully", "You can now view the updated status in the custom orders list.");
...
<template>
  <div>
    <h2 class="text-center font-semibold border-grey border-b pb-4 mb-4">
      {{ $t('artistProfile.comission.declineOrder') }}
    </h2>
    <div class="pb-16 border-b ">
      <p class="mb-4">
        {{ $t('artistProfile.comission.declineOrderContent') }}
      </p>
      <textarea
          id=""
          v-model="form.text"
