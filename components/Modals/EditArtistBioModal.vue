<script setup>
import {useVuelidate} from "@vuelidate/core";
import {minLength, required} from "@vuelidate/validators";
import {storeToRefs} from "pinia";
import {useModalsStore} from "~/store/modals.js";

const modals = useModalsStore()

const artistProfileStore = useArtistProfileStore();
const {editArtistData, getArtistData} = artistProfileStore;
const {form} = storeToRefs(artistProfileStore);

const loading = ref(false);

const v$ = useVuelidate(
    {
      artist_about: {required, minLength: minLength(40)},
    },
    form,
);

const onClick = async () => {
  await v$.value.$validate();
  if (v$.value.$error) return;
  loading.value = true;
  await editArtistData()
  await getArtistData();
  loading.value = false;
  modals.modal.show = false;
};
</script>

<template>
  <div class="modal-box">
    <h2 class="text-center font-semibold border-grey border-b pb-4 mb-4">
      {{ $t("artistRegistration.modals.editAbout.title") }}
    </h2>
    <form
        class="flex flex-col gap-2"
        @submit.prevent="onClick">
      <div class="flex flex-col mb-10">
            <textarea
                v-model="form.artist_about"
                :class="{
                'border-red-500':
                  v$.artist_about.$error && v$.artist_about.$dirty,
              }"
                class="w-full p-4 border    rounded-2xl focus:outline-none mb-2"
                rows="5"
            ></textarea>
        <div
            class="flex justify-between items-center text-sm text-secondaryText"
        >
          <p v-if="form.artist_about">
            {{ form.artist_about.length }} / 40
          </p>
          <p>
            {{
              $t("artistRegistration.modals.editAbout.minimumCharacters")
            }}
          </p>
        </div>
      </div>
    </form>
    <div class="mt-5">
      <div class="w-full flex justify-between gap-3">
        <form class="w-full" method="dialog">
          <button
              class="w-full bg-gray-200 py-2 rounded-xl"
              @click="modals.modal.show = false">
            {{ $t("artistRegistration.buttons.cancel") }}
          </button>
        </form>
        <div class="w-full">
          <p
              v-if="loading === false"
              class="cursor-pointer w-full bg-black text-white text-center py-2 rounded-xl"
              @click="onClick"
          >
            {{ $t("artistRegistration.buttons.save") }}
          </p>
          <div
              v-else
              class="cursor-pointer w-full bg-black text-white text-center py-2 rounded-xl"
          >
            <div class="spinner"/>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>
