...
const mergedArray = computed(() => {
  ...
  if (artOptions.resultSubjects && artOptions.resultSubjects.data) {
    artOptions.resultSubjects.data.forEach(item => {
      mergedSet.add(item);
    });
  }
...
const v$ = useVuelidate(
    {
      artist_art_subjects: {required, minLength: minLength(1)},
    },
    form,
);
...
<template>
  <div class="modal-box">
    ...
    <form class="flex flex-col gap-2">
      <div v-if="artOptions.resultSubjects">
        ...
        <div class="flex flex-wrap gap-4">
          <p
              ...
              :key="index"
              :class="{ '!bg-black text-white' : artistProfileStore.form.artist_art_subjects.includes(item.id)}"
              class="w-max bg-gray-200 cursor-pointer capitalize hover:bg-black hover:text-white transition-all rounded-full px-5 py-2  "
              @click="addElementToArray(artistProfileStore.form.artist_art_subjects,item.id)">
