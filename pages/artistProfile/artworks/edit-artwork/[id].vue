...
const {formEdit} = storeToRefs(artistArtworksStore);
...
async function sendForm() {
  ...
  if (artistArtworksStore.dimensionType === "inch") {
    formEdit.value.art_item_params.size.width =
        formEdit.value.art_item_params.size.width * 2.54;
    formEdit.value.art_item_params.size.height =
        formEdit.value.art_item_params.size.height * 2.54;
    formEdit.value.art_item_params.size.length =
        formEdit.value.art_item_params.size.length * 2.54;
  }

  await api(`/api/art/art-item/edit-art-item-info/`, {
...
onMounted(async () => {
  ...
  formEdit.value.art_item_artist_id = result.value.data.id;
  formEdit.value.language = language.cur_lang;
});
</script>
